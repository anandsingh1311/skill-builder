#!/usr/bin/env python3
"""Re-create and seed the MOCK tracker database from scratch (deterministic).

Run before every simulation trial. Never touches the production tracker.db —
this builds ./tracker.db next to this script.
"""
import sqlite3
import sys
...
USERS = [
    ...
    (3, "Devon Cole", "devon@acme.dev", 0),   # suspended user — assignment must fail
...
def main():
    ...
    con.commit()
    con.close()
    print(f"Seeded {DB}", file=sys.stderr)
