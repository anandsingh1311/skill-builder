#!/usr/bin/env python3
"""Mock of the Acme project-tracker MCP server for testing.

Behaviorally identical to the production server (same tools, same quirks,
same error strings) but reads/writes a throwaway SQLite database whose path
comes from the TRACKER_DB environment variable. Test agents therefore can
never touch the real tracker.db.
"""
import os
import sqlite3
