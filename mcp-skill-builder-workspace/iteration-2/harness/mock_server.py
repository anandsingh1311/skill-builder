#!/usr/bin/env python3
"""Mock of the Acme tracker MCP server.

Tool-for-tool identical to the production fixture (same names, params, errors,
quirks) so a skill tuned against it transfers unchanged. The ONLY difference is
that it reads its database path from $MOCK_DB, so each simulation trial can point
it at its own throwaway DB. Never writes to production.
"""
import os
import re
import sqlite3
...
# Per-trial DB path injected by the simulator; local fallback for manual runs.
